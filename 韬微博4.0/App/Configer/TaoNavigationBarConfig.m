...
- (void)configBarItems {
    ...
    if (!item.rightBarButtonItem && self.rightButtonType != TaoNavBarButtonTypeCustom) {
        NSMutableArray* rightArray = [NSMutableArray new];
        if (_rightButtonTypeString == nil) {
            [rightArray addObject:[self navBarItemWithType:self.rightButtonType
                                                     title:self.rightButtonTitle
                                                      font:self.rightButtonFont
                                                  selector:@selector(navRightBtnClick:)]];
        }
        else{
            NSArray* typeArray = [_rightButtonTypeString componentsSeparatedByString:@"|"];
            for (NSString* str in typeArray) {
                [rightArray addObject:[self navBarItemWithType:[str integerValue]
                                                         title:self.rightButtonTitle
                                                          font:self.rightButtonFont
                                                      selector:@selector(navRightBtnClick:)]];
            }
        }
        switch (rightArray.count) {
            case 0:break;
            case 1:item.rightBarButtonItem = rightArray[0];break;
            default:item.rightBarButtonItems = rightArray;break;
        }
    }
}
