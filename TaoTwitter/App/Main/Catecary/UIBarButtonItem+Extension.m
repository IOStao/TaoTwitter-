...
+ (UIBarButtonItem *)itemWithTarget:(id)target action:(SEL)action type:(TaoNavBarButtonType)type title:(NSString *)title {
    ...
    switch (type) {
        ...
        case TaoNavBarButtonTypeIconMessage:
            ...
            break;
        case TaoNavBarButtonTypeCustomUser:
            resultButtonItem = [UIBarButtonItem customUserViewWithtarget:target action:action];
            break;
...
+ (UIBarButtonItem *)customUserViewWithtarget:(id)target action:(SEL)action {
    return [[UIBarButtonItem alloc] initWithCustomView:[UIImageView userimageWithTarget:target action:action]];
}
@end
