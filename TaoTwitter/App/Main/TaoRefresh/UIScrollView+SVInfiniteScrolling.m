...
@property (nonatomic, strong) UILabel *UpText;
...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.infiniteScrollingView) {
        ...
        SVInfiniteScrollingView *view = [[SVInfiniteScrollingView alloc] initWithFrame:CGRectMake(0, height, self.bounds.size.width, SVInfiniteScrollingViewHeight)];
...
- (void)setShowsInfiniteScrolling:(BOOL)showsInfiniteScrolling {
    ...
    if(!showsInfiniteScrolling) {
      ...
      if (!self.infiniteScrollingView.isObserving) {
        ...
        NSInteger height = MAX(self.contentSize.height, self.bounds.size.height);
        self.infiniteScrollingView.frame = CGRectMake(0, height, self.infiniteScrollingView.bounds.size.width, SVInfiniteScrollingViewHeight);
      }
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {    
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        [self layoutSubviews];
         NSInteger height = MAX(self.scrollView.contentSize.height, self.bounds.size.height);
        self.frame = CGRectMake(0, height, self.bounds.size.width, SVInfiniteScrollingViewHeight);
    }
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        NSInteger height = MAX(self.scrollView.contentSize.height, self.bounds.size.height);
        CGFloat scrollViewContentHeight = height;
...
- (void)setEnabled:(BOOL)enabled {
    _enabled = enabled;
    if (!enabled) {
        self.UpText.hidden = YES;
    }
}

- (UILabel *)UpText {
    if (!_UpText) {
        _UpText = [[UILabel alloc] init];
        _UpText.font = [UIFont systemFontOfSize:15];
        _UpText.textColor = [UIColor tao_textLableColor];
        _UpText.text = @"上拉加载更多";
        [self addSubview:_UpText];
        [_UpText mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (UIActivityIndicatorView *)activityIndicatorView {
    if(!_activityIndicatorView) {
        ...
        [_activityIndicatorView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.UpText.mas_right).offset(20);
